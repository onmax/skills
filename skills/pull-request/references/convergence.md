...
Apply the intent and consent contract in [`../SKILL.md`](../SKILL.md). The classifications below refine that contract without creating a new approval gate.

- Read-only intent: `check`, `review`, `status`, `inspect`, or a bare PR URL or number. Inspect and report without changing local files, git history, the branch, or GitHub state.
- Refinement intent: `refine`, `fix`, `address`, or equivalent explicit action language. Make scoped local edits, run relevant checks, commit, and ordinarily push to the existing PR branch. When thread resolution is authorized, resolve only threads that the pushed code demonstrably addresses.
- Comments or replies, PR body edits, labels, closing, merging, force-pushing, approvals or request-changes reviews, thread resolution, and the merge, rebase, or rewrite history strategy remain separate mutation classes.
...
- Use `ponytail:ponytail` when the PR feels too broad or needs a smaller correct implementation.
...
- Use `grill-with-docs` when the user explicitly requests an interview. Create durable project documentation only when they also request that artifact.
...
- Overbroad PR: run `ponytail:ponytail` before editing.
...
3. Verify that the chosen history strategy is already authorized. If it is not, ask once, then execute only that strategy.
...
After consent under the mutation contract, use `engineering-writing` for repository style, issue linking, evidence, and body content rules.
