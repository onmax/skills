...
This is the Onmax adaptation of Matt Pocock's `writing-great-skills` reference. For non-trivial edits, load the shared [GLOSSARY.md](../../writing-great-skills/GLOSSARY.md) and use its vocabulary for predictability, invocation, information hierarchy, and failure modes.
...
[Link to separate files, such as `REFERENCE.md`, only when that file exists.]
```
