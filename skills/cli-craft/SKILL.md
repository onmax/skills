...
Choose libraries by contract pressure, not reputation or a default stack. Start with the repository convention. Let a dependency earn its place by owning non-trivial semantics or edge cases in parsing and help, prompts, configuration, process execution, filesystem and path behavior, network protocols, or ecosystem detection.

Before adopting one, check:

- whether its model fits the command grammar instead of forcing the command into the framework;
- whether its API lets the executable edge own help and error rendering, asynchronous action failures, and process termination when stream and status semantics are part of the contract;
- runtime and module compatibility, types, maintenance, license, and security posture;
- install footprint, startup cost, transitive dependencies, and the package's bundle-versus-external strategy.

Use the library directly while its use is localized. Add an adapter only when it keeps process or runtime state out of domain work, normalizes multiple providers or formats, or protects a stable callable interface from a volatile dependency API. Keep parser, prompt, configuration, and rendering types out of reusable domain APIs unless that integration is intentionally public.

Keep named library recommendations project-local. Preserve the target ecosystem's choices unless command grammar, contract control, or packaging constraints justify a change.
