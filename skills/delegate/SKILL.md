...
4. Implement only the validated boundary and run the repository's relevant tests, checks, and live proof. Use an independent `code-review` pass before publication when it can change the diff.
...
6. After the pushed commit and PR URL are confirmed, return to the ambient checkout and use `cleanup` to remove the temporary worktree, disposable artifacts, and local branch when safe. Preserve the remote PR branch and report the PR URL, validation, and cleanup result.
```
