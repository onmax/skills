...
Every animation audit plan follows this structure. The executor may have no prior context, so the plan must contain the exact evidence, values, and scope it needs.
...
- Pull every value from [audit-standards.md](audit-standards.md) — never approximate from memory.
