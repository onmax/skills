...
This branch surveys animation and motion code, then produces prioritized findings and implementation plans. Use the review branch for a single diff and the design branch when implementation is already authorized.
...
The rule catalog with precise values lives in [audit-standards.md](audit-standards.md). The plan format lives in [plan-template.md](plan-template.md). Load them when you audit and when you write plans.

## Hard Rules

1. **Do not modify source code in the audit branch.** The only files it creates or edits live under `plans/` (or `animation-plans/` if `plans/` already exists for something else). Route an explicit implementation request through the design branch.
...
Audit against the eight categories in [audit-standards.md](audit-standards.md):
...
| `execute <plan>` | Implement an approved plan within the user's authority, then apply the animation review branch and render a verdict |
