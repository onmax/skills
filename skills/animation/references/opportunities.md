...
This branch sweeps an interface for moments that would genuinely benefit from motion and proposes a precise recipe for each. Use the review branch for existing motion, the audit branch for a broad plan, and the design branch for implementation.
...
1. **Do not modify source code in the opportunities branch.** Route an explicit implementation request through the design branch.
...
Close with how much motion the interface needs, whether it is already close to right, and which single suggestion has the highest leverage. Offer the audit branch when a suggestion needs a self-contained implementation plan.
