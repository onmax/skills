...
- ignore PRs with no new actionable signal unless they satisfy merge-ready preconditions or need branch freshness in a merge-allowed repo
...
Query discipline: first run metadata-only `gh pr list` for each repo with number, title, author, head, draft, merge state, updated time, and URL only. Do not fetch comments, reviews, or full check rollups for non-authored PRs. Fetch unresolved review threads for authored PRs before using merge state as a blocker. For authored clean/non-draft candidates, fetch only that PR's checks, review threads, latest commit, and PR-level reactions, then merge or record the exact blocker before moving to broad comment inspection.
