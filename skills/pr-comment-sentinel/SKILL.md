...
- fetch unresolved current review threads before merge-ready checks, including dirty or blocked authored PRs
...
- ignore PRs with no new actionable signal unless they satisfy merge-ready preconditions, need branch freshness, or need check repair in a merge-allowed repo
- for merge-allowed repos, treat `DIRTY` or `BEHIND` authored PRs with no unresolved current threads as actionable branch-freshness lanes
- for merge-allowed repos, treat failed or cancelled required checks on authored PRs with no unresolved current threads as actionable check-repair lanes
...
Query discipline: first run metadata-only `gh pr list` for each repo with number, title, author, head, draft, merge state, updated time, and URL only. Do not fetch comments, reviews, or full check rollups for non-authored PRs. Fetch unresolved review threads for authored PRs before using merge state as a blocker. Fetch check summaries and failed logs for authored merge-allowed PRs when no worker is alive for the current head. For authored clean/non-draft candidates, fetch only that PR's checks, review threads, latest commit, and PR-level reactions, then merge or record the exact blocker before moving to broad comment inspection.
