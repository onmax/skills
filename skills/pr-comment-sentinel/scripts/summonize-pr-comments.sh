...
10. Repository-specific merge policy: automatic merge is allowed for gh:vite-hub/vitehub, but disabled for gh:quiverdk/portal. For any no-merge repository, including gh:quiverdk/portal, never call gh pr merge, never enable auto-merge, never delete the branch, and never call gh pr ready automatically. If a no-merge PR satisfies every normal merge gate, report it as ready-for-human-review and stop; do not merge it. For merge-allowed repositories only: if a PR is authored by the current GitHub user, has a Conventional Commits title, is a draft, has a clean merge state, all required checks succeeded, no unresolved current review threads remain, ${skill_dir}/scripts/codex-command-blocker.sh <owner/repo> <pr> <current-head> exits 0, and either the latest Codex signal is a thumbs-up/no-major-issues signal for the current head, a PR-level Codex bot thumbs-up reaction was created after the latest commit, or an observable internal fallback review owner returned no-major-issues for the current head after comments are disabled or Codex review is unavailable, mark it ready for review with gh pr ready. Then immediately re-read the PR head, draft state, checks, review threads, and ${skill_dir}/scripts/codex-command-blocker.sh <owner/repo> <pr> <current-head> before deciding merge readiness. Merge without waiting only when the repository is merge-allowed, the blocker script exits 0, the PR is authored by the current GitHub user, has a Conventional Commits title, is not a draft, has a clean merge state, all required checks succeeded, no unresolved current review threads remain, and either the latest Codex signal is a thumbs-up/no-major-issues signal for the current head, a PR-level Codex bot thumbs-up reaction was created after the latest commit, or an observable internal fallback review owner returned no-major-issues for the current head after comments are disabled or Codex review is unavailable. Use the recent repository strategy: squash merge. Delete the PR branch when GitHub allows it for same-repo branches. Do not merge after needs-fix or inconclusive fallback verdicts, when no review owner was available, when the blocker script reports a newer @codex command, when new commits landed after the fallback review, or when the repository is no-merge. If a worker pushed a fix, do not wait for checks; revisit it in a later heartbeat after checks are already green, then run a fresh fallback review for that new head before considering merge.
