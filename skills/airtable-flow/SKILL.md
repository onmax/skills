...
Treat the PR body as a reviewer artifact, not the flow handoff. For Airtable work without a required repository template, use one outcome sentence, a compact `[Airtable task <ID>](<sourceUrl>)` link without a section heading or task title, then the smallest proof that establishes merge confidence. Keep commands, CI and deployment chronology, fixture setup, environment details, and operational limitations in the handoff unless they materially change merge confidence.
...
7. When self-review, relevant checks, and preview proof or its explicit limitation pass, mark the PR ready and hand it to Maxi for review. Route later feedback through `pull-request`.
