---
name: airtable-flow
description: Advances Quiver Airtable tasks through a gated selection-to-review lifecycle. Use when Maxi asks to pick, claim, investigate, implement, or finish Ready-to-implement Airtable work.
---
...
- Leave GitHub reviewers and assignees empty, mention nobody, and obtain the consent required by the target repository before any GitHub comment or reply.
...
3. Self-review the task, diff, repository rules, and preview behavior. Repair every in-scope finding and repeat affected checks before declaring the PR ready.
4. Mark the PR ready after the checks and preview proof pass, and leave reviewer assignment to Maxi.
5. Restore any isolated preview data changed during proof or explicitly hand off its remaining state, then tell Maxi the PR needs his manual second pass.

The flow is complete only when the PR is ready for Maxi's review, its body carries the decision-relevant proof, a live CLI read shows the automatic PR comment and status, and temporary reproduction state is accounted for. Route later review feedback through `pr-refiner`.
